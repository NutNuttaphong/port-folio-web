import { Link } from 'react-router-dom';
import { Mail, Phone, MapPin, } from 'lucide-react';

export default function Footer() {
  return (
    <footer className="bg-[#0b1329] text-slate-300 border-t border-white/10">
      <div className="max-w-7xl mx-auto px-6 py-12 grid grid-cols-1 md:grid-cols-4 gap-8">
        
        {/* Brand */}
        <div className="space-y-4">
          <div className="text-2xl font-bold text-white text-start tracking-wider">
            Port<span className="text-emerald-400">Dev</span>
          </div>
          <p className="text-sm text-slate-400 leading-relaxed">
            ผู้นำด้านการจัดงานอีเวนต์ระดับมืออาชีพ รังสรรค์ทุกช่วงเวลาสำคัญให้เป็นประสบการณ์ที่น่าจดจำ
          </p>
          
          {/* Social Icons (ใช้ SVG ตรงๆ ไม่พึ่ง library เพื่อป้องกัน Error) */}
          <div className="flex items-center gap-3 pt-2">
            {/* Facebook */}
            <a href="#" className="h-9 w-9 flex items-center justify-center rounded-full bg-slate-800 hover:bg-emerald-500 hover:text-white transition">
              <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
              </svg>
            </a>

            {/* Instagram */}
            {/* <a href="#" className="h-9 w-9 flex items-center justify-center rounded-full bg-slate-800 hover:bg-emerald-500 hover:text-white transition">
              <Instagram size={18} />
            </a> */}

            {/* X / Twitter */}
            <a href="#" className="h-9 w-9 flex items-center justify-center rounded-full bg-slate-800 hover:bg-emerald-500 hover:text-white transition">
              <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
              </svg>
            </a>

            {/* LinkedIn */}
            <a href="#" className="h-9 w-9 flex items-center justify-center rounded-full bg-slate-800 hover:bg-emerald-500 hover:text-white transition">
              <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
              </svg>
            </a>
          </div>
        </div>

        {/* Quick Links */}
        <div>
          <h4 className="text-white font-semibold mb-4 tracking-wider text-start text-sm">เมนูนำทาง</h4>
          <ul className="space-y-2 text-start text-sm">
            <li><Link to="/portfolio" className="hover:text-emerald-400 transition">Portfolio</Link></li>
            <li><Link to="/about-us" className="hover:text-emerald-400 transition">About Us</Link></li>
            <li><Link to="/inspiration" className="hover:text-emerald-400 transition">Inspiration</Link></li>
            <li><Link to="/event/services" className="hover:text-emerald-400 transition">Our Services</Link></li>
            <li><Link to="/booking" className="hover:text-emerald-400 transition">Book Now</Link></li>
          </ul>
        </div>

        {/* Services */}
        <div>
          <h4 className="text-white font-semibold mb-4 tracking-wider text-start text-sm">บริการ</h4>
          <ul className="space-y-2 text-start text-sm text-slate-400">
            <li className="hover:text-slate-200 transition">Product Launches</li>
            <li className="hover:text-slate-200 transition">Corporate Conferences</li>
            <li className="hover:text-slate-200 transition">Wedding Gala</li>
            <li className="hover:text-slate-200 transition">Virtual & Hybrid Events</li>
          </ul>
        </div>

        {/* Contact */}
        <div>
          <h4 className="text-white font-semibold mb-4 tracking-wider text-start text-sm">ติดต่อเรา</h4>
          <ul className="space-y-3 text-sm text-slate-400">
            <li className="flex items-center gap-3">
              <MapPin size={18} className="text-emerald-400 shrink-0" />
              <span>Bangkok, Thailand</span>
            </li>
            <li className="flex items-center gap-3">
              <Phone size={18} className="text-emerald-400 shrink-0" />
              <span>+66 (0) 2 123 4567</span>
            </li>
            <li className="flex items-center gap-3">
              <Mail size={18} className="text-emerald-400 shrink-0" />
              <span>contact@eventco.com</span>
            </li>
          </ul>
        </div>

      </div>

      <div className="border-t border-white/5 bg-[#080d1c] py-6 px-6 text-center text-xs text-slate-500 flex flex-col sm:flex-row justify-between items-center mx-auto gap-4">
        <p>© 2026 EventCo. All rights reserved.</p>
        <div className="flex gap-6">
          <a href="#" className="hover:text-slate-400 transition">Privacy Policy</a>
          <a href="#" className="hover:text-slate-400 transition">Terms of Service</a>
        </div>
      </div>
    </footer>
  );
}